...
public class AiService {
    ...
    public QuizDto parseAiResponse(String aiResponse, String name, String topic, String description) {
        ...
        String[] questionsArray = content.split("\\d+\\.");
        for (String question : questionsArray) {
            log.info("Question in questionsArray: {}", question);
        }
        ...
        QuizDto quizDto = QuizDto.builder()
                ...
                .questionIds(new HashSet<>()) // Set for storing question IDs
                .userId(user.getId())
                .build();

        List<QuestionDto> questionDtos = new ArrayList<>();

        // Iterate over question blocks and save each question
        for (String questionBlock : questionsArray) {
            ...
            // Save the QuestionDto as a Question entity
            Question questionEntity = questionMapper.toEntity(questionDto); // Use your mapper to convert to entity
            Question savedQuestion = questionService.save(questionEntity); // Save the question and get the saved entity
                                                                           // with ID

            // Add saved question ID to quizDto
            quizDto.getQuestionIds().add(savedQuestion.getId());

            // Add the saved question to the list (or handle as needed)
            questionDtos.add(questionDto);
        }

        return quizDto; // Return the quiz with associated question IDs
    }
